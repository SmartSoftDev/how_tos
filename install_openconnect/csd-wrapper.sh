...
export OPENSSL_CONF=/home/ghe/ghub/ssd/how_tos/install_openconnect/openssl_unsafe_legacy_renegotiation.conf
env
# getting manifest, and checking binaries
wget --no-check-certificate -O "$HOSTSCAN_DIR/manifest" "${URL}/sdesktop/hostscan/$ARCH/manifest"  || fatal "could not get manifest file!"
...
while read HASHTYPE FILE EQU HASHVAL; do
	...
	*)
	...
	TMPFILE="${PATHNAME}.tmp"
        ...
        wget --no-check-certificate  -O "${TMPFILE}" "${URL}/sdesktop/hostscan/$ARCH/$FILE"
        ...
        then
            ...
            FILE_GZ="${FILE}.gz"
            wget --no-check-certificate  -O "${TMPFILE}.gz" "${URL}/sdesktop/hostscan/$ARCH/$FILE_GZ"  &&
